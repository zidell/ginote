...
개인적으로 GitHub Issues를 좋아하는데 너무 굼뜨고 UX가 불편한 점이 항상 불만이었습니다.
그래서 기능은 거의 그대로 가져오되 사용자 경험만 노트앱처럼 만들어주는 SPA 앱을
만들었습니다. 이 앱은 정적 JS로만 구성되어 있으며 브라우저에서 GitHub API로 직접
통신하므로 안전합니다.
...
- **음성 녹음·전사·정제 (OpenAI 한정):** 브라우저에서 녹음한 음성을 OpenAI로 직접
  ...
  보존합니다. 현재 음성 기능은 OpenAI API만 지원합니다.
...
- **키보드를 통한 제어:** 노트 목록 이동·열기·선택, 새 노트 작성, 저장소 전환
  등을 키보드로 할 수 있습니다.
- **MCP 연동:** GitHub 공식 MCP Server를 연결하면 AI 도구에서도 같은 노트(이슈)를
  읽고 쓸 수 있습니다. 별도의 앱 전용 MCP 서버는 필요 없습니다.
