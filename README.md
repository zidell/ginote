...
- **태그·검색·휴지통:** GitHub 라벨을 태그로 사용하며 본문 검색과 닫힌 이슈 기반
  휴지통을 지원합니다.
- **키보드를 통한 제어:** 노트 목록 이동·열기·선택, 새 노트 작성, 워크스페이스
  전환 등을 키보드로 할 수 있습니다.
...
첨부는 전용 댓글이나 메타데이터 마커 없이, 이슈 본문에 직접 Markdown 링크로
연결됩니다. 이미지는 미리보기 이미지 링크, 그 외 파일은 📎 파일 링크로
삽입됩니다.
...
편집기에서는 반복되는 `https://github.com/{저장소}/raw/HEAD/` 부분을
`{repo}/`로 압축해서 보여주고, GitHub에 실제로 저장하는 시점에만 완전한
주소로 다시 펼칩니다.

첨부 작업은 다음 순서로 처리됩니다.

- 추가: 저장소에 파일 생성 → 본문 끝에 Markdown 링크 삽입 (이미지·클립보드
  붙여넣기·드래그 앤 드롭은 본문 편집 영역 전체가 대상입니다)
- 첨부 뷰어의 **본문삽입**: 목록에는 있지만 본문에 링크가 없는 파일을 본문
  맨 끝에 즉시 삽입
- 삭제: 첨부 목록에서 명시적으로 지운 파일만 저장소 파일과 본문 링크를 함께
  제거. 본문에서 링크만 지우고 첨부 목록에서 지우지 않은 파일은 저장소에
  그대로 남아 목록에도 계속 보이며, 다음 저장 시점에 그 링크를 본문 앞쪽에
  조용히 되살립니다 (현재 열려 있는 편집 화면 자체는 건드리지 않고, 저장되는
  내용에만 반영합니다)
- 복구: 이슈를 열 때 그 이슈의 파일 폴더만 읽어 본문에 있는 링크와 경로를
  대조합니다. 링크가 있는 파일은 "연결된 첨부", 없는 파일은 "아직 저장 전인
  고아 첨부"로 구분해 목록에 표시합니다

다른 이슈나 저장소 전체를 스캔하지 않습니다. 첨부는 노트당 최대 30개, 파일
하나당 10MB로 제한합니다. 첨부 기능을 사용하려면 PAT에 Contents의 Read and
write 권한이 필요합니다. `.issue-note-assets/`의 파일 경로나 본문의 첨부
Markdown 링크 형식을 GitHub에서 직접 수정하면 앱의 첨부 연결이 깨질 수
있습니다.
