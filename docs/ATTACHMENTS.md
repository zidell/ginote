...
이미지·파일 링크는 GitHub Issue에서도 그대로 표시됩니다. 사용자가 **본문삽입**으로
...
- **추가:** 저장소에 파일을 만든 뒤 첨부 목록에 표시합니다. 본문 첨부는 이슈 폴더에,
  댓글 첨부는 해당 댓글 ID 전용 하위 폴더에 저장합니다. 댓글은 먼저 저장되어 GitHub
  댓글 ID가 있어야 파일을 첨부할 수 있습니다. 저장할 때 본문에 직접
  넣지 않은 첨부는 GitHub 호환을 위해 본문 최상단의 관리 블록에 자동으로 연결합니다.
  댓글도 같은 관리 블록 방식을 자체 본문에 사용하며, 댓글 첨부가 노트 본문 목록이나
  본문 관리 블록에 들어가지는 않습니다.
  이미지·클립보드 붙여넣기·드래그 앤 드롭은 본문 편집 영역 어디에서나 사용할 수
  있습니다.
- **본문에 넣기:** 크게보기의 **Markdown 복사**로 링크를 원하는 곳에 붙여넣거나,
  **본문삽입**으로 본문 맨 끝에 직접 연결할 수 있습니다.
...
`parseAttachmentPaths`는 본문에서 위 링크 형식을 계속 인식합니다. 자동 첨부 블록은
`ginote:attachments:start`와 `ginote:attachments:end` 주석 경계로만 구분하며,
편집기에서는 이 영역을 숨기고 저장 시 다시 생성합니다. 저장소 파일은 있지만 본문에
직접 넣은 링크가 없는 고아 첨부는 명시적으로 삭제하기 전까지 목록에서 지우면 안
됩니다.
