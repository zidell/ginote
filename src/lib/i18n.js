...
const common = {
  ...
  es: { Add: 'Añadir', Attach: 'Adjuntar', 'Back to list': 'Volver a la lista', Clear: 'Borrar', Close: 'Cerrar', Copy: 'Copiar', Delete: 'Eliminar', Download: 'Descargar', 'Editor settings': 'Ajustes del editor', Font: 'Fuente', List: 'Lista', 'Load more': 'Cargar más', 'Loading notes…': 'Cargando notas…', 'Manage tags': 'Gestionar etiquetas', 'New note': 'Nueva nota', 'New tag name': 'Nueva etiqueta', Notes: 'Notas', 'Notes per page': 'Notas por página', Refresh: 'Actualizar', Restore: 'Restaurar', Saved: 'Guardado', 'Saving...': 'Guardando...', Search: 'Buscar', Settings: 'Ajustes', Tags: 'Etiquetas', Trash: 'Papelera', 'Auto-save delay': 'Retraso del guardado automático', 'Line height': 'Interlineado', Size: 'Tamaño', 'Title mode': 'Modo de título', 'Write your note…': 'Escribe tu nota…', 'Select a note from the list.': 'Selecciona una nota de la lista.', 'No notes yet.': 'Aún no hay notas.', 'No results found.': 'No se encontraron resultados.', 'Search or #tag': 'Buscar o #etiqueta', 'Connect and start': 'Conectar y empezar', 'Connect a GitHub repository': 'Conectar un repositorio de GitHub', 'Remember PAT in this browser': 'Recordar el PAT en este navegador', 'Notes repository': 'Repositorio de notas' , sec: 's' , 'Changed.': 'Modificado.' }
};

const dynamic = {
  en: { noteCount: '{count} notes', openNote: 'Open note {title}', clearFilter: 'Clear {label} filter', tagName: '{name} tag name', createTag: 'Create #{name}', tagAdded: 'Added #{name}.', tagDeleted: 'Deleted #{name}.', deleteTagConfirm: 'Delete the #{name} tag from every note?', tagRenamed: 'Renamed #{from} to #{to}.', tagNameRequired: 'The name of #{name} cannot be empty.', selectOwner: 'Select the {owner} account.', selectRepository: 'Select only {repo}.', createPat: 'Create a PAT for {name}', allocateFailed: 'Could not allocate a number for the new note. {error}', moveToTrashConfirm: 'Move “{title}” to trash?', restoreConfirm: 'Restore “{title}”?', closedIssueSaveConfirm: '“{title}” was moved to trash on another device. Restore it and save your changes?', attachmentLimit: 'Each note can have up to {count} attachments.', attachmentLimitAdded: 'Only {count} attachments were added because that is the per-note limit.', fileTooLarge: '“{name}” was not uploaded because it exceeds 10 MB.', deleteAttachmentConfirm: 'Delete “{name}” from the repository too?', deleteAttachment: 'Delete attachment {name}', attachmentDeleting: 'Deleting…', uploading: 'Uploading ({count})', removeTag: 'Remove tag {name}', openLink: 'Open {url} in a new tab', pasteLocationRequired: 'Place the cursor where you want to paste first.', createdAt: 'Created: {date}', updatedAt: 'Updated: {date}', viewOnGitHub: 'View on GitHub', workspaceDisplayNameLabel: 'Display name for {repo}', mcpPrompt: 'Use GitHub Issues in {repo} as notes. Open issues are regular notes, closed issues are trash, and labels are tags. Each attachment is stored under .issue-note-assets/issues/{issueNumber}/ and referenced by a plain image or file link directly in the issue body. Preserve these links when editing a note\'s body — deleting one detaches that attachment.' },
...
const meta = {
  ...
  es: { description: 'Notas sencillas, guardadas como GitHub Issues', sourceCode: 'Ver el código fuente en GitHub' }
};
const settings = {
  en: { language: 'Language', theme: 'Theme', themeDark: 'Dark (default)', themeLight: 'Light', sidebarLabel: 'Settings', patReplacementPlaceholder: 'Enter only to replace the saved PAT', backgroundRefreshInterval: 'Check for new notes', refreshDisabled: 'Off', minutes: 'minutes', hours: 'hours', workspaceCacheDuration: 'Keep workspace information for', workspaceCacheHelp: 'Restores the list state when you return within this time. It is cleared when the app reloads.', lockSessionDuration: 'Remember lock code for (cleared on refresh)', lockSessionHelp: 'How long an entered lock number stays reusable in this browser before locked notes ask for it again.', editorMaxWidth: 'Content max width' },
  ko: { language: '언어', theme: '화면 모드', themeDark: '다크모드 (기본)', themeLight: '라이트모드', sidebarLabel: '설정', patReplacementPlaceholder: '저장된 PAT를 바꿀 때만 입력하세요', backgroundRefreshInterval: '새 노트 확인 간격', refreshDisabled: '사용 안 함', minutes: '분', hours: '시간', workspaceCacheDuration: '저장소 전환 후 목록을 기억하는 시간', workspaceCacheHelp: '이 시간 안에 돌아오면 마지막으로 보던 목록을 다시 보여줍니다. 앱을 새로고침하면 지워집니다.', lockSessionDuration: '잠금 숫자 기억 시간 (새로고침하면 지워짐)', lockSessionHelp: '입력한 잠금 숫자를 이 브라우저에서 다시 묻지 않고 사용할 시간입니다. 시간이 지나면 잠긴 노트를 열 때 다시 입력해야 합니다.', editorMaxWidth: '본문 최대 너비' },
  'zh-CN': { language: '语言', theme: '主题', themeDark: '深色（默认）', themeLight: '浅色', sidebarLabel: '设置', patReplacementPlaceholder: '仅在更改已保存的 PAT 时输入', backgroundRefreshInterval: '检查新笔记', refreshDisabled: '关闭', minutes: '分钟', hours: '小时', lockSessionDuration: '锁定数字记忆时长（刷新页面后自动清除）', lockSessionHelp: '设置已输入的锁定数字在本浏览器中可重复使用的时长。超时后，锁定笔记会再次要求输入。', editorMaxWidth: '正文最大宽度' },
  ja: { language: '言語', theme: 'テーマ', themeDark: 'ダーク（デフォルト）', themeLight: 'ライト', sidebarLabel: '設定', patReplacementPlaceholder: '保存済みのPATを変更する場合のみ入力', backgroundRefreshInterval: '新しいノートの確認間隔', refreshDisabled: 'オフ', minutes: '分', hours: '時間', lockSessionDuration: 'ロック番号の記憶時間（更新時に自動解除）', lockSessionHelp: '一度入力したロック番号をこのブラウザで再利用できる時間です。経過するとロックされたノートで再入力を求められます。', editorMaxWidth: '本文の最大幅' },
  de: { language: 'Sprache', theme: 'Erscheinungsbild', themeDark: 'Dunkel (Standard)', themeLight: 'Hell', sidebarLabel: 'Einstellungen', patReplacementPlaceholder: 'Nur zum Ändern des gespeicherten PAT eingeben', backgroundRefreshInterval: 'Nach neuen Notizen suchen', refreshDisabled: 'Aus', minutes: 'Minuten', hours: 'Stunden', lockSessionDuration: 'Sperrzahl merken für (wird beim Neuladen gelöscht)', lockSessionHelp: 'Wie lange eine eingegebene Sperrzahl in diesem Browser wiederverwendbar bleibt, bevor gesperrte Notizen erneut danach fragen.', editorMaxWidth: 'Maximale Inhaltsbreite' },
  fr: { language: 'Langue', theme: 'Thème', themeDark: 'Sombre (par défaut)', themeLight: 'Clair', sidebarLabel: 'Paramètres', patReplacementPlaceholder: 'Saisir uniquement pour remplacer le PAT enregistré', backgroundRefreshInterval: 'Rechercher de nouvelles notes', refreshDisabled: 'Désactivé', minutes: 'minutes', hours: 'heures', lockSessionDuration: 'Mémoriser le code de verrouillage pendant (effacé au rechargement)', lockSessionHelp: 'Durée pendant laquelle un code de verrouillage saisi reste réutilisable dans ce navigateur avant que les notes verrouillées le redemandent.', editorMaxWidth: 'Largeur maximale du contenu' },
  it: { language: 'Lingua', theme: 'Tema', themeDark: 'Scuro (predefinito)', themeLight: 'Chiaro', sidebarLabel: 'Impostazioni', patReplacementPlaceholder: 'Inserisci solo per sostituire il PAT salvato', backgroundRefreshInterval: 'Controlla nuove note', refreshDisabled: 'Disattivato', minutes: 'minuti', hours: 'ore', lockSessionDuration: 'Ricorda il codice di blocco per (cancellato al ricaricamento)', lockSessionHelp: 'Per quanto tempo un codice di blocco inserito resta riutilizzabile in questo browser prima che le note bloccate lo richiedano di nuovo.', editorMaxWidth: 'Larghezza massima del contenuto' },
  es: { language: 'Idioma', theme: 'Tema', themeDark: 'Oscuro (predeterminado)', themeLight: 'Claro', sidebarLabel: 'Ajustes', patReplacementPlaceholder: 'Introdúcelo solo para reemplazar el PAT guardado', backgroundRefreshInterval: 'Buscar notas nuevas', refreshDisabled: 'Desactivado', minutes: 'minutos', hours: 'horas', lockSessionDuration: 'Recordar el código de bloqueo durante (se borra al recargar)', lockSessionHelp: 'Tiempo durante el cual un código de bloqueo introducido se puede reutilizar en este navegador antes de que las notas bloqueadas vuelvan a pedirlo.', editorMaxWidth: 'Ancho máximo del contenido' }
};
const setup = {
  en: {
  ...
  es: {
    confirmPatStorage: '¿Guardar este PAT en este navegador?\n\nSi lo guardas, la app podrá volver a conectarse sin pedírtelo cada vez. Cualquier persona que pueda usar este perfil del navegador podría usar el PAT, así que nunca actives esta opción en un dispositivo público o compartido.\n\n¿Es este tu dispositivo personal y quieres continuar? Si eliges «Cancelar», no se cambiará nada ni se establecerá ninguna conexión. Después podrás desmarcar la casilla y conectarte sin guardar el PAT.',
    wizardTitle: 'Configurar Ginote', stepCount: 'Paso {step} de {total}', progressLabel: 'Progreso de la configuración',
    introTitle: 'Guarda tus notas personales en tu propio repositorio de GitHub', introDescription: 'Ginote convierte GitHub Issues en una app de notas sencilla. Tus notas viajan directamente entre este navegador y GitHub, sin pasar por ningún servidor de la app.',
    introFeatureStorage: 'Cada nota se guarda como un GitHub Issue.', introFeatureTags: 'Las etiquetas de GitHub sirven como etiquetas y los issues cerrados, como papelera.', introFeaturePrivacy: 'Un repositorio privado evita que tus notas personales sean públicas.',
    accountTitle: 'Primero, prepara una cuenta de GitHub', accountDescription: 'Necesitas una cuenta de GitHub para crear y guardar notas. Crear una cuenta de GitHub es gratis. Si aún no tienes una, créala desde el siguiente enlace.', createAccount: 'Crear una cuenta de GitHub',
    repositoryTitle: 'Crea un repositorio privado para tus notas', repositoryDescription: 'Crea un repositorio nuevo en GitHub y asegúrate de que su visibilidad sea Private. Luego pega su dirección abajo.', createRepository: 'Crear un repositorio privado en GitHub', repositoryTitleWorkspace: 'Elige el repositorio que vas a usar', repositoryDescriptionWorkspace: 'Si ya tienes acceso a un repositorio, introduce su dirección abajo. También puedes crear uno nuevo si lo necesitas.', createRepositoryWorkspace: 'Crear un repositorio nuevo en GitHub', repositoryAddress: 'Dirección del repositorio', repositoryAddressHelp: 'Puedes introducir una URL de GitHub u owner/repository.',
    patTitle: 'Crea e introduce un PAT', patDescription: 'Ginote necesita un fine-grained personal access token para leer y escribir notas en este repositorio.', createPat: 'Crear un PAT para este repositorio', patHelp: 'Concede acceso de lectura y escritura a Issues. Para usar archivos adjuntos, concede también acceso de lectura y escritura a Contents.', rememberPat: 'Recordar el PAT en este navegador',
    previous: 'Atrás', start: 'Empezar la configuración', haveAccount: 'Ya tengo una cuenta', repositoryDone: 'Dirección introducida', finish: 'Completar la configuración', connecting: 'Conectando…', cancel: 'Cancelar', close: 'Cerrar'
  }
};
...
const workspace = {
  ...
  es: { switcherLabel: 'Cambiar de espacio de trabajo', addWorkspace: 'Añadir otro repositorio', settingsSectionTitle: 'Espacios de trabajo', settingsSectionHelp: 'Elige el orden en que aparecen los espacios de trabajo o deja uno que ya no necesites.', moveUp: 'Subir', moveDown: 'Bajar', currentBadge: 'Actual', rename: 'Cambiar nombre', leave: 'Revocar acceso', leaveConfirm: '¿Revocar el acceso a {repo}? La dirección del repositorio y el token de acceso guardados se eliminarán de este navegador y tendrás que volver a conectarte más adelante.', leftNotice: 'Se revocó el acceso a {repo}.', switchToManageTags: 'Cambia a este espacio de trabajo para gestionar sus etiquetas.', switchAction: 'Cambiar', moreActions: 'Más acciones' }
};

const help = {
  en: {
    buttonSecurity: 'Security guide', buttonMcp: 'MCP guide', buttonApp: 'Install the app', buttonKeyboard: 'Keyboard shortcuts',
    keyboardTitle: 'Keyboard shortcuts', keyboardIntro: 'Most of these shortcuts work while the note list is active. Ctrl/Cmd+R reloads the app from anywhere.', keyboardMove: 'Move through notes', keyboardOpen: 'Open the focused note; press again to start editing', keyboardNew: 'Create a new note', keyboardRefresh: 'Reload the app', keyboardWorkspaceMenu: 'Open the workspace picker; use ↑/↓ to move and Enter to switch', keyboardWorkspace: 'Switch workspace (only in the note list; registered workspaces 1~9)', keyboardEscape: 'Clear the selection, cancel a deletion, or close this guide', keyboardSelect: 'Select the focused note', keyboardRangeSelect: 'Select a range of notes', keyboardTrash: 'Move selected notes to trash', keyboardNoteActions: 'With a note open and no field focused: R reloads the whole app, S saves the note, T adds a tag, A attaches a file, P pins, L locks or unlocks, E opens replace, Delete moves it to trash, G opens it on GitHub, and M opens the MD viewer.', keyboardNote: 'Most shortcuts do not run while you are typing in a field. Ctrl/Cmd+R is the browser reload shortcut; R, S, and E work on an open note when no field is focused.',
  ...
  'zh-CN': {
    ...
    keyboardTitle: '键盘快捷键', keyboardIntro: '大多数快捷键可在笔记列表中使用。Ctrl/Cmd+R 可随时刷新应用。', keyboardMove: '在笔记间移动', keyboardOpen: '打开当前笔记；再次按下开始编辑', keyboardNew: '新建笔记', keyboardRefresh: '刷新应用', keyboardWorkspaceMenu: '打开工作区选择器；使用 ↑/↓ 移动并按 Enter 切换', keyboardWorkspace: '切换工作区（仅限笔记列表；已注册工作区的 1~9）', keyboardEscape: '取消选择、取消删除或关闭此说明', keyboardSelect: '选中当前笔记', keyboardRangeSelect: '选择一组笔记', keyboardTrash: '将选中的笔记移至回收站', keyboardNoteActions: '打开笔记且没有输入框获得焦点时：R 刷新整个应用，S 保存笔记，T 添加标签，A 附加文件，P 置顶，L 锁定或解锁，E 打开替换窗口，Delete 移入回收站，G 在 GitHub 中打开，M 打开 MD 查看器。', keyboardNote: '在输入框中输入时，大多数快捷键不会生效。Ctrl/Cmd+R 是浏览器的刷新快捷键；打开笔记且没有输入框获得焦点时，R 刷新整个应用，S 保存笔记，E 打开替换窗口。',
    securityTitle: '安全说明',
  ...
  es: {
    buttonSecurity: 'Resumen de seguridad', buttonMcp: 'Guía de MCP', buttonApp: 'Instalar la app', buttonKeyboard: 'Atajos de teclado',
    keyboardTitle: 'Atajos de teclado', keyboardIntro: 'La mayoría de estos atajos funcionan en la lista de notas. Ctrl/Cmd+R recarga la app en cualquier momento.', keyboardMove: 'Moverse entre las notas', keyboardOpen: 'Abrir la nota activa; pulsa de nuevo para editarla', keyboardNew: 'Crear una nota nueva', keyboardRefresh: 'Recargar la app', keyboardWorkspaceMenu: 'Abrir el selector de espacios de trabajo; usa ↑/↓ para moverte y Intro para cambiar', keyboardWorkspace: 'Cambiar de espacio de trabajo (solo en la lista de notas; espacios registrados 1~9)', keyboardEscape: 'Quitar la selección, cancelar una eliminación o cerrar esta guía', keyboardSelect: 'Seleccionar la nota activa', keyboardRangeSelect: 'Seleccionar un rango de notas', keyboardTrash: 'Mover las notas seleccionadas a la papelera', keyboardNote: 'La mayoría de los atajos no funcionan mientras escribes en un campo. Ctrl/Cmd+R es el atajo del navegador para recargar la página.',
    securityTitle: 'Resumen de seguridad',
    securityIntro: 'Ginote es una app web completamente estática. No hay ningún servidor de la app que gestione el inicio de sesión ni guarde tus notas: una vez abierta la app, toda la comunicación se produce directamente entre este navegador y GitHub.',
    securityDiagramFlow: 'Este navegador  ←── comunicación directa ──→  GitHub',
    securityDiagramNote: 'El PAT, los ajustes de la app y los borradores sin guardar se quedan solo en este navegador',
    securityPoint1: 'Las notas, etiquetas y archivos adjuntos se guardan solo en el repositorio de GitHub que elijas.',
    securityPoint2: 'El PAT y los ajustes de la app se quedan solo en este navegador y se envían a la API de GitHub únicamente para la autenticación.',
    securityPoint3: 'Los borradores locales sin guardar se quedan solo en este navegador.',
    securityPoint4: 'Si quieres, puedes cifrar el cuerpo de una nota con AES-GCM en el navegador antes de enviarla.',
    securityPoint5: 'No existe ninguna API que envíe tus notas, tu PAT o tus ajustes al operador de la app, y no se usa ningún servicio de analítica ni de seguimiento.',
    securityOutro: 'Aparte de la solicitud habitual para descargar los archivos estáticos, no se envía ningún dato adicional a otro servidor que no sea GitHub. Tus notas solo existen realmente en este navegador y en el repositorio de GitHub que elegiste.',
    securityLinkLabel: 'Leer la explicación completa en el README del proyecto',
    mcpIntro: 'Añade el servidor MCP oficial de GitHub a una herramienta de IA compatible con MCP para leer y escribir los mismos GitHub Issues que usas aquí como notas.',
    appTitle: 'Instalar la app',
    appIntro: 'Ginote se puede instalar como una app nativa en lugar de quedarse en una pestaña del navegador.',
    appPwaSectionTitle: 'Instalar como PWA',
    appPwaChromeDesktop: 'Chrome/Edge (escritorio): haz clic en el icono de instalación del extremo derecho de la barra de direcciones o elige "Instalar Ginote…" en el menú del navegador.',
    appPwaAndroid: 'Chrome para Android: abre el menú del navegador (⋮) y toca "Instalar app" o "Añadir a pantalla de inicio".',
    appPwaIOS: 'Safari en iPhone/iPad: toca el botón Compartir y elige "Añadir a pantalla de inicio".',
    appPwaMacSafari: 'Safari en macOS (Sonoma o posterior): abre el menú Compartir y elige "Añadir al Dock".',
    appPwaNote: 'El service worker solo almacena en caché los archivos estáticos de la propia app; nunca las respuestas de la API de GitHub, tu PAT ni los datos de tus notas.',
    appDesktopSectionTitle: 'App de escritorio',
    appDesktopIntro: 'También hay una app de escritorio nativa para macOS, Windows y Linux. Igual que la versión web, se conecta directamente a la API de GitHub sin ningún servidor adicional de la app.',
    appDesktopHomebrew: 'En macOS también se puede instalar con Homebrew:',
    appDesktopLinkLabel: 'Descargar desde GitHub Releases'
  }
};
