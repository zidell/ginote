...
const help = {
  ...
  'zh-CN': {
    ...
    keyboardTitle: '键盘快捷键', keyboardIntro: '大多数快捷键可在笔记列表中使用。Ctrl/Cmd+R 可随时刷新应用。', keyboardMove: '在笔记间移动', keyboardOpen: '打开当前笔记；再次按下开始编辑', keyboardNew: '新建笔记', keyboardRefresh: '刷新应用', keyboardWorkspaceMenu: '打开工作区选择器；使用 ↑/↓ 移动并按 Enter 切换', keyboardWorkspace: '切换工作区（仅限笔记列表；已注册工作区的 1~9）', keyboardEscape: '取消选择、取消删除或关闭此说明', keyboardSelect: '选中当前笔记', keyboardRangeSelect: '选择一组笔记', keyboardTrash: '将选中的笔记移至回收站', keyboardNoteActions: '打开笔记且没有输入框获得焦点时：R 刷新整个应用，S 保存笔记，T 添加标签，A 附加文件，P 置顶，L 锁定或解锁，Delete 移入回收站，G 在 GitHub 中打开，M 打开 MD 查看器。', keyboardNote: '在输入框中输入时，大多数快捷键不会生效。Ctrl/Cmd+R 是浏览器的刷新快捷键；打开笔记且没有输入框获得焦点时，R 刷新整个应用，S 保存笔记。',
    securityTitle: '安全说明',
