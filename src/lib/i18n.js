...
const dynamic = {
  ...
  ko: { noteCount: '{count}개', openNote: '{title} 노트 열기', clearFilter: '{label} 필터 해제', tagName: '{name} 태그 이름', createTag: '#{name} 새로 만들기', tagAdded: '#{name} 태그를 추가했습니다.', tagDeleted: '#{name} 태그를 삭제했습니다.', deleteTagConfirm: '#{name} 태그를 모든 노트에서 삭제할까요?', tagRenamed: '#{from} 태그를 #{to}(으)로 변경했습니다.', tagNameRequired: '#{name} 태그 이름을 비워둘 수 없습니다.', selectOwner: '{owner} 계정이 선택되어야 합니다.', selectRepository: '{repo} 저장소 하나만 선택하세요.', createPat: '{name}용 PAT 발급하기', allocateFailed: '새 노트 번호를 만들지 못했습니다. {error}', moveToTrashConfirm: '“{title}” 노트를 휴지통으로 이동할까요?', restoreConfirm: '“{title}” 노트를 복원할까요?', attachmentLimit: '첨부파일은 노트당 최대 {count}개까지 추가할 수 있습니다.', attachmentLimitAdded: '첨부파일은 노트당 최대 {count}개까지만 추가했습니다.', fileTooLarge: '“{name}”은 10MB를 초과하여 올리지 않았습니다.', deleteAttachmentConfirm: '“{name}” 파일을 저장소에서도 삭제할까요?', deleteAttachment: '{name} 첨부 삭제', uploading: '업로드 중 ({count})', removeTag: '{name} 태그 제거', openLink: '{url} 새 탭에서 열기', pasteLocationRequired: '붙여넣을 위치를 먼저 지정하세요.', createdAt: '생성: {date}', updatedAt: '수정: {date}', viewOnGitHub: 'GitHub에서 보기', workspaceDisplayNameLabel: '{repo}의 표시명', mcpPrompt: '{repo} 저장소의 GitHub Issues를 노트로 사용해줘. 열린 이슈는 일반 노트, 닫힌 이슈는 휴지통이며 라벨은 태그야. 각 첨부파일은 .issue-note-assets/issues/{issueNumber}/ 폴더에 저장되고, 이슈 본문 안에 평범한 이미지·파일 링크로 직접 연결돼 있어. 노트 본문을 편집할 때 이 링크들은 그대로 유지해줘 — 링크를 지우면 그 첨부가 떨어져 나가.' }
};
...
const settings = {
  en: { language: 'Language', sidebarLabel: 'Settings', patReplacementPlaceholder: 'Enter only to replace the saved PAT', backgroundRefreshInterval: 'Check for new notes', refreshDisabled: 'Off', minutes: 'minutes', hours: 'hours', lockSessionDuration: 'Keep the lock number for', lockSessionHelp: 'How long an entered lock number stays reusable in this browser before locked notes ask for it again.', editorMaxWidth: 'Content max width' },
  ko: { language: '언어', sidebarLabel: '설정', patReplacementPlaceholder: '저장된 PAT를 변경할 때만 입력하세요', backgroundRefreshInterval: '새 노트 확인 주기', refreshDisabled: '사용 안 함', minutes: '분', hours: '시간', lockSessionDuration: '잠금 숫자 유지 시간', lockSessionHelp: '한 번 입력한 잠금 숫자를 이 브라우저에서 얼마나 재사용할지 정합니다. 시간이 지나면 잠금 노트가 숫자를 다시 묻습니다.', editorMaxWidth: '본문 최대 너비' },
  'zh-CN': { language: '语言', sidebarLabel: '设置', patReplacementPlaceholder: '仅在更改已保存的 PAT 时输入', backgroundRefreshInterval: '检查新笔记', refreshDisabled: '关闭', minutes: '分钟', hours: '小时', lockSessionDuration: '锁定数字保留时间', lockSessionHelp: '设置已输入的锁定数字在本浏览器中可重复使用的时长。超时后，锁定笔记会再次要求输入。', editorMaxWidth: '正文最大宽度' },
  ja: { language: '言語', sidebarLabel: '設定', patReplacementPlaceholder: '保存済みのPATを変更する場合のみ入力', backgroundRefreshInterval: '新しいノートの確認間隔', refreshDisabled: 'オフ', minutes: '分', hours: '時間', lockSessionDuration: 'ロック番号の保持時間', lockSessionHelp: '一度入力したロック番号をこのブラウザで再利用できる時間です。経過するとロックされたノートで再入力を求められます。', editorMaxWidth: '本文の最大幅' },
  de: { language: 'Sprache', sidebarLabel: 'Einstellungen', patReplacementPlaceholder: 'Nur zum Ändern des gespeicherten PAT eingeben', backgroundRefreshInterval: 'Nach neuen Notizen suchen', refreshDisabled: 'Aus', minutes: 'Minuten', hours: 'Stunden', lockSessionDuration: 'Sperrzahl behalten für', lockSessionHelp: 'Wie lange eine eingegebene Sperrzahl in diesem Browser wiederverwendbar bleibt, bevor gesperrte Notizen erneut danach fragen.', editorMaxWidth: 'Maximale Inhaltsbreite' },
...
const setup = {
  ...
  ko: {
    ...
    repositoryTitle: '비공개 노트 저장소를 만드세요', repositoryDescription: 'GitHub에서 새 저장소를 만들고 공개 범위를 반드시 Private으로 설정하세요. 만든 뒤 저장소 주소를 아래에 붙여 넣습니다.', createRepository: 'GitHub에서 비공개 저장소 만들기', repositoryTitleWorkspace: '함께 사용할 저장소를 지정하세요', repositoryDescriptionWorkspace: '이미 참여 중인 저장소가 있다면 그 주소를 아래에 입력하세요. 필요하다면 새 저장소를 만들 수도 있습니다.', createRepositoryWorkspace: 'GitHub에서 새 저장소 만들기', repositoryAddress: '저장소 주소', repositoryAddressHelp: 'GitHub 주소 또는 owner/repository 형식으로 입력할 수 있습니다.',
    ...
    previous: '이전', start: '설정 시작', haveAccount: '계정이 있습니다', repositoryDone: '주소 입력 완료', finish: '설정 완료', connecting: '연결 중…', cancel: '취소', close: '닫기'
  },
  'zh-CN': {
...
const workspace = {
  ...
  ko: { switcherLabel: '워크스페이스 전환', addWorkspace: '다른 저장소 추가', settingsSectionTitle: '워크스페이스', settingsSectionHelp: '전환 시 표시되는 순서를 정하거나, 더 이상 쓰지 않는 워크스페이스에서 나갈 수 있습니다.', moveUp: '위로 이동', moveDown: '아래로 이동', currentBadge: '현재 사용 중', rename: '표시명 변경', leave: '인증해제', leaveConfirm: '{repo}의 인증을 해제할까요? 이 브라우저에 저장된 저장소 주소와 접근 토큰이 삭제되며, 나중에 다시 인증해야 합니다.', leftNotice: '{repo}의 인증을 해제했습니다.', switchToManageTags: '이 워크스페이스로 전환해야 태그를 관리할 수 있습니다.', switchAction: '전환', moreActions: '더보기' },
...
const help = {
  en: {
    buttonSecurity: 'Security guide', buttonMcp: 'MCP guide', buttonApp: 'Install the app',
  ...
  ko: {
    buttonSecurity: '보안 안내', buttonMcp: 'MCP 안내', buttonApp: '앱 설치',
    securityTitle: '보안 안내',
    ...
    securityPoint2: 'PAT와 앱 설정은 이 브라우저에만 보관되며, 인증을 위해서만 GitHub API로 전달됩니다.',
    securityPoint3: '저장 전 로컬 초안은 이 브라우저 안에만 남습니다.',
    securityPoint4: '원하면 노트 본문을 브라우저 안에서 AES-GCM으로 암호화한 뒤 전송할 수 있습니다.',
    securityPoint5: '앱 운영자에게 노트·PAT·설정을 보내는 API가 없으며, 분석이나 추적 서비스도 사용하지 않습니다.',
    securityOutro: '정적 파일을 내려받는 일반적인 접속 요청 외에는 GitHub 이외의 서버로 추가 데이터가 전송되지 않습니다. 실제 노트 데이터가 존재하는 곳은 이 브라우저와 사용자가 선택한 GitHub 저장소뿐입니다.',
    securityLinkLabel: '프로젝트 README에서 자세한 설명 보기',
    mcpIntro: 'MCP를 지원하는 AI 도구에 GitHub의 공식 MCP Server를 설치하면, 여기서 노트로 쓰는 것과 같은 GitHub Issues를 AI 도구에서도 읽고 쓸 수 있습니다.',
    appTitle: '앱 설치',
    appIntro: 'Ginote는 브라우저 탭 대신 설치형 앱처럼 사용할 수 있습니다.',
    appPwaSectionTitle: 'PWA로 설치하기',
    ...
    appPwaNote: '서비스 워커는 앱 자체의 정적 파일만 캐시하며, GitHub API 응답이나 PAT, 노트 데이터는 캐시하지 않습니다.',
  ...
  'zh-CN': {
    buttonSecurity: '安全说明', buttonMcp: 'MCP 说明', buttonApp: '安装应用',
    securityTitle: '安全说明',
  ...
  ja: {
    buttonSecurity: 'セキュリティについて', buttonMcp: 'MCPガイド', buttonApp: 'アプリのインストール',
  ...
  de: {
    buttonSecurity: 'Sicherheitsübersicht', buttonMcp: 'MCP-Anleitung', buttonApp: 'App installieren',
  ...
  fr: {
    buttonSecurity: 'Aperçu de la sécurité', buttonMcp: 'Guide MCP', buttonApp: 'Installer l’application',
  ...
  it: {
    buttonSecurity: 'Panoramica sulla sicurezza', buttonMcp: 'Guida MCP', buttonApp: "Installa l'app",
...
addMessages('zh-CN', { ...zh, dynamic: dynamicFor('zh-CN'), errors: errors.en, meta: meta['zh-CN'], settings: settings['zh-CN'], setup: setup['zh-CN'], workspace: workspace['zh-CN'], help: help['zh-CN'] });
for (const code of ['ja', 'de', 'fr', 'it']) {
  addMessages(code, { ...catalogWithOverrides(common[code]), dynamic: dynamicFor(code), errors: errors.en, meta: meta[code], settings: settings[code], setup: setup[code], workspace: workspace[code], help: help[code] });
}
